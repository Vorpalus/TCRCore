...
public abstract class TCRSoundProvider extends SoundDefinitionsProvider {
    ...
    protected TCRSoundProvider(PackOutput output, ExistingFileHelper helper) {
        super(output, TCRCoreMod.MOD_ID, helper);
    }
    ...
    public void generateNewSound(RegistryObject<SoundEvent> event, String baseSoundDirectory, int numberOfSounds, boolean subtitle) {
        ...
        if (subtitle) {
            ...
            definition.subtitle("subtitles." + TCRCoreMod.MOD_ID + "." + splitSoundName[0] + "." + splitSoundName[2]);
        }
        for (int i = 1; i <= numberOfSounds; i++) {
            definition.with(SoundDefinition.Sound.sound(ResourceLocation.fromNamespaceAndPath(TCRCoreMod.MOD_ID, baseSoundDirectory + (numberOfSounds > 1 ? "_" + i : "")), SoundDefinition.SoundType.SOUND));
        }
    ...
    public void makeMusicDisc(RegistryObject<SoundEvent> event, String discName) {
        this.add(event, SoundDefinition.definition()
                .with(SoundDefinition.Sound.sound(ResourceLocation.fromNamespaceAndPath(TCRCoreMod.MOD_ID, "music/" + discName), SoundDefinition.SoundType.SOUND)
                        .stream()));
    }
